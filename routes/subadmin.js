...
//                 }else{
//                     console.log('Login success!')
//                     res.redirect('/scholarships/dashboard')
//                 }
//             }else{
//                 //รอทำแจ้งเตือน
//                 let loginStatus = 'Password is Incorrect!'
...
//         res.render('subadmins/subadminlist',{admindata:doc})
//     })
// })
...
router.get('/list',(req,res) => {
    Scholarship.find().limit(80).exec().then((doc,err) => {
        res.render('subadmins/scholarshiplist',{scholarships:doc})
    })
})
...
router.get('/insertdata', async (req, res, next) => {
    ...
    let l_branch = await Major.distinct('branch').exec()
        res.render('subadmins/insertscholarship',{scholarshipstype:l_stype,l_facu:l_facu,l_branch:l_branch})
  })
...
router.use('/result', async (req, res) => {
    ...
    await Scholarship.find({
        ...
        res.render('subadmins/resultfilter',{scholarships:doc})
...
router.post('/edit',(req,res) => {
    ...
    Scholarship.findOne({_id:edit_id}).exec().then((doc,err) => {
        res.render('subadmins/editscholarship',{scholarships:doc})
    })
})
...
router.get('/userlist',(req,res) => {
    User.find().exec().then((doc,err) => {
        res.render('subadmins/userlist',{userdata:doc})
    })
})
...
router.get('/thisuser/:id',(req,res) => {
    ...
    User.findOne({_id:this_id}).exec().then((doc,err) => {
        console.log(doc)
        res.render('subadmins/thisuser',{userdata:doc})
    })
})
...
router.get('/this/:id',(req,res) => {
    ...
    Scholarship.findOne({_id:this_id}).exec().then((doc,err) => {
        console.log(doc)
        res.render('subadmins/thisscholarship',{scholarships:doc})
    })
})
...
router.post('/selectbranch', async (req,res) =>{
    ...
    if(faculty_list != ''){
        res.render('subadmins/deletebranch',{faculty_list:faculty_list,selcter:selcter})
    }else{
...
//-UI- หน้าเพิ่มข้อมูล ประเภท คณะ สาขา ทุน
router.get('/addcomp', async(req,res)=>{
    ...
    res.render('subadmins/addcomp',{faculty:majordoc,listtype:listtype,facultys:majordocs})
